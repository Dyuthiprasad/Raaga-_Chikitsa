...
class KanakangiScreen extends StatefulWidget {
  const KanakangiScreen({super.key});

  @override
  State<KanakangiScreen> createState() => _KanakangiScreenState();
}

class _KanakangiScreenState extends State<KanakangiScreen> {
  ...
  Future<void> setAudioByIndex(int index) async {
    try {
      await audioPlayer.setSource(AssetSource('audio/M1/M1($index).wav'));
      if (_mounted) {
        setState(() {
          isAudioLoaded = true;
        });
      ...
      print('Error loading audio asset M1($index).wav: $e');
      if (_mounted) {
        setState(() {
          isAudioLoaded = false;
        });
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildBenefitRow('Balances restlessness and promotes inner calm.'),
              _buildBenefitRow('Supports a focused mind and reduces mental distractions.'),
              _buildBenefitRow('Fosters feelings of devotion and spiritual connection.'),
              _buildBenefitRow('Encourages a sense of stability and grounding.'),
              const SizedBox(height: 16),
              Text('Physical Health Benefits:', style: TextStyle(color: brown, fontSize: screenWidth * 0.05)),
              const SizedBox(height: 8),
              _buildBenefitRow('Eases stress-related physical tension.'),
              _buildBenefitRow('Supports mindful breathing and respiratory awareness.'),
              _buildBenefitRow('Contributes to overall well-being through mental serenity.'),
            ],
          ),
        ),
      ),
